...
Thanks for using `openfec-mcp-server`. Bugs, feature requests, and documentation gaps all belong in an issue — that's where they get read and picked up.

Open one from the **Issues** tab and pick the [**Bug Report**](https://github.com/cyanheads/openfec-mcp-server/issues/new?template=bug_report.yml) or [**Feature Request**](https://github.com/cyanheads/openfec-mcp-server/issues/new?template=feature_request.yml) form. Both are structured, and filling in the fields is what makes an issue actionable.

PRs welcome; open an issue first for anything larger than a typo.

## Server bug or framework bug?

`openfec-mcp-server` is built on [@cyanheads/mcp-ts-core](https://github.com/cyanheads/mcp-ts-core), which handles transports, auth, config, logging, and telemetry. Sorting out which layer broke saves everyone a round-trip:
...
Upstream data problems are a third case: a missing field, a candidate or committee ID that no longer resolves, or a filing that never appears usually reflects what [api.open.fec.gov](https://api.open.fec.gov/developers/) returned. Include the tool arguments and the FEC IDs involved so the raw response can be checked.
...
Don't open a public issue for a vulnerability. Report it privately — GitHub's **Security** tab → **Report a vulnerability**, or email **security@caseyjhand.com**. Full policy: [SECURITY.md](./SECURITY.md).
